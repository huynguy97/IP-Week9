...
{
    counter_equal++;
    return ( a.artist==b.artist && a.cd==b.cd && a.year==b.year && a.time.minutes==b.time.minutes && a.time.seconds==b.time.seconds) ;
...
int main()
{
    ...
    */
    ...
    for ( int i=100; i < MAX_NO_OF_SONGS; i=i+100)
    {
        counter_smaller = 0 ;
        counter_equal = 0 ;
        const int NO_OF_SONGS = read_file("Tracks.txt") ;
        insertion_sort(songs, i) ;
        int c = counter_smaller/100000 ;
        for (int j=1; j<c;j++)
        {
            cout << '*' ;
        }

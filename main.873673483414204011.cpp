...
{
    counter++;
    return (a.time < b.time || (a.time == b.time && a.artist < b.artist) || (a.time == b.time && a.artist == b.artist && a.title < b.title)  ||
...
void slice_comparisons (vector<Track>& data)
{
    cout<<"Insertion sort: "<<"\n";
    for(int length = 100; length <= 5800; length += 100 )
    {
        counter = 0;
        ...
        int quotient;
        quotient = counter/100000;

        for(int i = 0; i <= quotient; i++)
            cout<<"*";
        if(counter-quotient!=0)
            cout<<".";
        cout<<"\n";

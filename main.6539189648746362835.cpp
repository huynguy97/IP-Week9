...
bool operator<(const Track& a, const Track& b)
{
    ...
    if (a.time == b.time && a.artist < b.artist)
        return true;
    if (a.time == b.time && a.artist == b.artist && a.cd < b.cd)
        return true;
...
int calculate_vectorsize (int length)
{//               
    assert (true) ;
//                  
//                                                                     
    int slicenumber = length/100 ;
    if (length % 100 != 0)
        slicenumber++ ;
    return slicenumber ;
}

void bubbleslice (vector<int> slicescomparison, int length)
{//               
    assert (true) ;
//                  
//                                                                                 
    ...
    while (no < length)
    {
        while (!bubble ( songs, mkSlice (no, no+99 ) ))
        no = no + 100 ;
        slicescomparison [sl] = counter ;
        sl++ ;
    }
}

void selectionslice (vector<int> slicescomparison, int length)
{

}

void insertionslice (vector<int> slicescomparison, int length)
{

}

void read_array (vector<int> slicescomparison, int length)
{
...
int main()
{
    ...
    int vectorsize = calculate_vectorsize (NO_OF_SONGS) ;
    vector<int> slicescomparison (vectorsize);
    switch (m)
    {

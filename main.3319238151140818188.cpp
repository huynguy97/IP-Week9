...
*/
    char seperate;
    in >> length.minutes >> seperate >> length.seconds;
    return in;
}
...
{
    cout << "What sorting method do you want to use? " << "\n" ;
    for (int m = 0; m < NoOfSortingMethods; m++)
        cout << m+1 << ": " << methods[m] << " sort" << "\n" ;
    int choice ;
    cin >> choice ;
...
int main()
{
	...
	if (NO_OF_SONGS < 0)
	{
        cout << "Reading file failed. Program terminates." << "\n";
	    return NO_OF_SONGS;
    }

    SortingMethod m = get_sorting_method () ;

    cout << "Sorting tracks with " << methods[m] << " sort" << "\n";
    for (int count = 0; count + 100 <= NO_OF_SONGS; count += 100) {
        vector<Track> subgroup(songs.begin(), songs.begin() + count);
        count2 = 0;
        switch (m)
        {
            case InsertionSort: insertion_sort(subgroup, count) ; break ;
            case SelectionSort: selection_sort(subgroup, count) ; break ;
            case BubbleSort:    bubble_sort   (subgroup, count) ; break ;
            default:        cout << "Huh?" << "\n" ;
        }

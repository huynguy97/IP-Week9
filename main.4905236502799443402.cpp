...
//       
    for(int to = 99; to <= 5799; to=to+100)
    {
        counter = 0;
        vector<Track> sliced_tracks;
        copy_vector(sliced_tracks,songs,{0,to});
        switch (m)
        {
            case InsertionSort: insertion_sort(sliced_tracks,to) ; break ;
            case SelectionSort: selection_sort(sliced_tracks,to) ; break ;
            case BubbleSort:    bubble_sort   (sliced_tracks,to) ; break ;
            default:        cout << "Huh?" << endl ;
        }
        remainder = counter%100000;
        nr_stars = (counter-remainder)/100000;
        cout << string(nr_stars, '*');
        if(remainder>0)
            cout << '.';
        cout << endl;
    }

...
int main()
{
    ...
    for(int i=99; i < MAX_NO_OF_SONGS; i += 100)
    {
        vector<Track> songslice;
        for (int j=0; j < i; j++)
        {
            songslice.push_back(songs[j]);
            switch (m)
            {
                case InsertionSort: insertion_sort(songslice,size(songslice)) ; break ;
                case SelectionSort: selection_sort(songslice,size(songslice)) ; break ;
                case BubbleSort:    bubble_sort   (songslice,size(songslice)) ; break ;

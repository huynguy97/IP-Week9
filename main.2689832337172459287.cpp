...
void selection_sort (vector<Track>& data)
{
    ...
    {
        counter = 0 ;
        for ( int unsorted = 0 ; unsorted < i ; unsorted++ )
        {
            const int k = smallest_value_at (data, mkSlice (unsorted, i-1));
...
void bubble_sort (vector<Track>& data)
{
    ...
    {
        counter = 0 ;
        while ( !bubble ( data, mkSlice (0, i-1 ) ) )
        {
            i-- ;
        }
        display_steps() ;
...
int main()
{
    ...
    cout << "The number of steps is " << counter << endl ;

...
int main()
{
    ...
    for(int s=99;s<NO_OF_SONGS;s+=100){
        Slice slice = mkSlice(0,s);
        vector<Track> sngs;
        for(int i=slice.from;i<slice.to;i++){
            sngs.push_back(songs[i]);
        }
        switch (m)
        {
            case InsertionSort: insertion_sort(sngs,s) ; break ;
            case SelectionSort: selection_sort(sngs,s) ; break ;
            case BubbleSort:    bubble_sort   (sngs,s) ; break ;
    ...
    cout << "Number of comparisons made: " << counter;
	return 0;
}
